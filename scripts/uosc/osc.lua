...
local osc_styles = {
    ...
    rjno1timecodes = "{\\blur0\\bord0\\1c&HC7A25A\\3c&HFFFFFF\\fs13}",
    rjno1timePosBar = "{\\blur2\\bord".. user_opts.tooltipborder .."\\1c&HFFFFFF\\3c&H000000\\fs24}",
    ...
    smallButtonsLlabel = "{\\fscx105\\fscy105\\fs22\\fn" .. mp.get_property("options/osd-font") .. "}",
...
function prepare_elements()
    ...
    for _,element in pairs(elements) do
        ...
        elseif (element.type == "slider") then
            ...
            local seekbar_chapter_lo_gap = 10 --slider_lo.gap
            ...
            -- marker nibbles
            if not (element.slider.markerF == nil) and (seekbar_chapter_lo_gap > 0) then
                        ...
                        if false then -- draw triangles
                            ...
                            if (slider_lo.nibbles_top) then
                                static_ass:rect_cw(s - 1, slider_lo.border,
                                    s + 1, slider_lo.border + seekbar_chapter_lo_gap);
                            end

                            --bottom
                            if (slider_lo.nibbles_bottom) then
                                static_ass:rect_cw(s - 1,
                                    elem_geo.h -slider_lo.border -seekbar_chapter_lo_gap,
            ...
            if pos then
                ...
                if slider_lo.stype ~= "bar" then
                    if seekRanges then
                        ...
                        r = r - (innerH / 4) -- make elapsed seekbar less thick
...
--end	
	...
	geo ={x = rjno1X + 1100 / 2 , y = rjno1Y + 6+8+3 + 14 , an = 5, w = 1080, h = 1}
...
function visibility_mode(mode, no_osd)
    ...
    if mode == "uosc-enter" then
        uosc_mode = user_opts.visibility
        mode = "never"
    elseif mode == "uosc-leave" then
        mode = uosc_mode
    end

    if mode == "auto" then
        enable_osc(true)
        always_on(not no_osd)

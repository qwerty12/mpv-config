-- https://github.com/open-dynaMIX/simple-mpv-webui/blob/main/main.lua @ 8a80013bfab334a68f923973d61f03bfa0a75772
require 'mp.options'
require 'mp.msg'
...
local allowed_ips = options.allowed_ips ~= "" and {} or nil
if allowed_ips ~= nil then
  for i in options.allowed_ips:gmatch("([^,]+)") do
    allowed_ips[#allowed_ips + 1] = i
  end
end
...
local endpoints = {
  ...
  ["api/speed_adjust"] = {
    POST = function(request)
      ...
      local _, success, ret = pcall(mp.commandv, 'osd-msg', 'add', 'speed', amount)
      return handle_post(success, ret)
    end
  },
  ...
  ["api/sleep"] = {
    POST = function(_)
      ...
      ffi.cdef[[
      int SetSuspendState(int bHibernate, int bForce, int bWakeupEventsDisabled);
      ]]
  ...
  ["api/revert_seek"] = {
    POST = function(_)
      local _, success, ret = pcall(mp.commandv, 'osd-msg', "revert-seek")
      return handle_post(success, ret)
    end
  }
}
...
local function parse_request(connection)
  ...
  if allowed_ips ~= nil and not has_value(allowed_ips, request.clientip) then
    mp.msg.log("WARN", "Rejected connection from " .. request.clientip)
    return
  end
...
options.htpasswd_path = mp.command_native({'expand-path', options.htpasswd_path})
